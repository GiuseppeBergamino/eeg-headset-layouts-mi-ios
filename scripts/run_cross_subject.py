...
DIR_CSV = PROJECT_ROOT / "results" / "cross_subject"
DIR_CSV.mkdir(parents=True, exist_ok=True)

csv_summary_path = DIR_CSV / "__summary_CrossSub.csv"
...
# ULTRACORTEX_16, ULTRACORTEX_8, 
# GTEC_UNICORN_HYBRID, 
# BITBRAIN_DIADEM, BITBRAIN_AIR, BITBRAIN_HERO, BITBRAIN_IKON, 
# MUSE_2_HEADBAND, MUSE_S_ATHENA 
# NEUROSITY_CROWN,
...
mask_name = headset.name     # print_catalog()
channel_mask = headset.make_mask(available_channels, verbose = True)
n_channels = len(channel_mask)  
...
pipelines = {} # Dictionary of pipelines passed to CrossSubjectEvaluation.process.
...
# ------------------ PARADIGMS: ROC-AUC DEFAULT + CUSTOM MACRO-F1 ------------------
paradigm_auc = MotorImagery(
    n_classes = 2, # with two classes, MOABB uses ROC-AUC by default
    ...
    channels = channel_mask,  # mask is applied here
    resample = None
)

paradigm_f1 = MotorImageryF1( # custom macro-F1 scoring
    n_classes = 2, 
    events = ["left_hand", "right_hand"],
    fmin = 8.0,
    fmax = 32.0,
    ...
    tmax = 3.0,
    baseline = None,
    channels = channel_mask,  # same mask
    resample = None
)
...
n_jobs = 1
#------- RUN ROC-AUC (default)
eval_auc = CrossSubjectEvaluation(
    paradigm = paradigm_auc,
    datasets = [dataset],
    random_state = seed,
    n_jobs = n_jobs, # use -1 for all available cores
    overwrite = True,  # otherwise MOABB may reuse an existing HDF5 result file
    suffix = f"{exp_id}_AUC", # stored by MOABB under its results directory
)

res_auc = eval_auc.process(pipelines) # run evaluation

#-------- RUN F1 (custom scoring)
eval_f1 = CrossSubjectEvaluation(
...
res_f1 = eval_f1.process(pipelines) # run evaluation
...
summary_row = {
    ...
    #"roc_auc": f"{auc_mean:.2f} ± {auc_std:.2f}",
    #"f1_macro": f"{f1_mean:.2f} ± {f1_std:.2f}",
    #"moabb_version": getattr(moabb, "__version__", "NA"),
    #"mne_version": getattr(mne, "__version__", "NA"),
    #"sklearn_version": getattr(sklearn, "__version__", "NA"),
}
